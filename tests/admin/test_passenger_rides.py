"""
Test for admin passenger-rides command.

This test:
1. Sets up test data (admin, passenger, driver, locations, rides)
2. Tests the admin passenger-rides command with various options
...
class TestAdminPassengerRidesCommand(unittest.TestCase):
    """Test the 'cabcab admin passenger-rides' command."""
    ...
    def create_test_data(cls):
        """Create test users, drivers, and rides."""
        ...
        ride_2 = {
            ...
            "request_time": (datetime.now() - timedelta(days=1)).isoformat(),
            "start_time": None,
            "end_time": None,
            "status": "CANCELLED",
    ...
    def test_passenger_rides_command_table_format(self):
        """Test the 'cabcab admin passenger-rides' command with table output format."""
        # Login as admin
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(mode='w+') as temp_output:
                ...
                command = [
                    "python", "-c", 
                    f"import sys; sys.path.insert(0, '.'); from app.cli_module.commands.admin_commands import passenger_rides; passenger_rides('{PASSENGER_EMAIL}', None, 'table')"
                ]
                ...
                self.assertIn(f"Rides for passenger", output)
    ...
    def test_passenger_rides_command_detailed_format(self):
        """Test the 'cabcab admin passenger-rides' command with detailed output format."""
        # Login as admin
        mock_login, mock_verify, mock_require = self.login_as_admin()
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(mode='w+') as temp_output:
                ...
                command = [
                    "python", "-c", 
                    f"import sys; sys.path.insert(0, '.'); from app.cli_module.commands.admin_commands import passenger_rides; passenger_rides('{PASSENGER_EMAIL}', None, 'detailed')"
                ]
    ...
    def test_passenger_rides_command_with_status_filter(self):
        """Test the 'cabcab admin passenger-rides' command with status filter."""
        # Login as admin
        mock_login, mock_verify, mock_require = self.login_as_admin()
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(mode='w+') as temp_output:
                ...
                command = [
                    "python", "-c", 
                    f"import sys; sys.path.insert(0, '.'); from app.cli_module.commands.admin_commands import passenger_rides; passenger_rides('{PASSENGER_EMAIL}', 'COMPLETED', 'table')"
                ]
    ...
    def test_passenger_rides_command_with_invalid_email(self):
        """Test the 'cabcab admin passenger-rides' command with an invalid email."""
        # Login as admin
        mock_login, mock_verify, mock_require = self.login_as_admin()
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(mode='w+') as temp_output:
                ...
                command = [
                    "python", "-c", 
                    f"import sys; sys.path.insert(0, '.'); from app.cli_module.commands.admin_commands import passenger_rides; passenger_rides('invalid@test.com', None, 'table')"
                ]
