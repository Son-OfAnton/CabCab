...
class AuthService:
    ...
    def register_passenger(email: str, password: str, first_name: str, last_name: str, phone: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'password' not in created_user:
                logger.warning(
                    f"Password field missing from response for user {email}")
                ...
                try:
                    ...
                    if check_response.status_code == 200:
                        ...
                        if 'password' not in saved_user:
                            logger.error(
                                "Password field missing from saved user!")

                            # Attempt to update the user to add the password
                            update_user = saved_user.copy()
                            update_user['password'] = hashed_password
                            update_response = requests.put(
                                f"{BASE_URL}/users/{user_id}", json=update_user)
                            if update_response.status_code == 200:
                                logger.info(
                                    "Successfully added password to user record")
                            else:
                                logger.error(
                                    f"Failed to update user with password: {update_response.status_code}")
                except Exception as e:
            ...
            # Generate token
            token = AuthService._generate_jwt(
                user_id, UserType.PASSENGER.value)
            ...
            if 'password' not in created_user:
                ...
                try:
                    ...
                    pass_response = requests.post(
                        f"{BASE_URL}/passwords", json=password_data)
                    if pass_response.status_code == 201:
                        logger.info(
                            f"Saved password for user {email} in separate collection")
                except Exception as e:
                    logger.error(
                        f"Failed to save password in separate collection: {str(e)}")
    ...
    def login(email: str, password: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if user.get('user_type') == UserType.PASSENGER.value and user.get('is_banned', False):
                ...
                ban_type = "permanently" if user.get(
                    'is_permanent_ban', False) else "temporarily"
                raise AuthError(
                    f"Your account has been {ban_type} banned. Reason: {reason}. Please contact customer support for assistance.")

            # Check if password is missing in the user record
            if 'password' not in user or not user.get('password'):
                logger.warning(
                    f"Password field missing for user {email}, looking in passwords collection")

                # Try to get password from passwords collection
                try:
                    pass_response = requests.get(
                        f"{BASE_URL}/passwords/query?email={email}")
                    if pass_response.status_code == 200:
                        passwords = pass_response.json()
                        if passwords:
                            password_record = passwords[0]
                            hashed_password = password_record.get(
                                'hashed_password')
                            if hashed_password:
                                # Verify password using the stored hash
                                if AuthService._verify_password(password, hashed_password):
                                    logger.info(
                                        f"Password verified from passwords collection for {email}")
                                    ...
                                    try:
                                        ...
                                        if update_response.status_code == 200:
                                            logger.info(
                                                f"Updated user record with password for {email}")
                                    except Exception as e:
                                        logger.error(
                                            f"Failed to update user record: {str(e)}")
                                else:
                                    raise AuthError("Invalid password")
                except Exception as e:
                    logger.error(
                        f"Error checking passwords collection: {str(e)}")
                    raise AuthError("Invalid credentials")

            # If we still don't have a valid password field, authentication fails
            if 'password' not in user or not user.get('password'):
                raise AuthError(
                    "Account data is missing password information. Please register again or contact support.")
        ...
        try:
            ...
            ptype_field_restrictions = {
                ...
                UserType.ADMIN.value: [
                    'payment_methods', 'license_number', 'is_verified', 'is_available', 'vehicle_id']
            }
            ...
            response = requests.put(
                f"{BASE_URL}/users/{user_id}", json=updated_user)
            response.raise_for_status()
    ...
    def require_user_type(token: str, required_types: List[str]) -> Dict[str, Any]:
        ...
        if user_type not in required_types:
            ...
            raise AuthError(
                f"Access denied. This action requires one of these user types: {allowed_types}")
    ...
    try:
        ...
        try:
            ...
            if ban_status.get('is_banned'):
                ...
                permanent = "permanently" if ban_status.get(
                    'is_permanent') else "temporarily"
